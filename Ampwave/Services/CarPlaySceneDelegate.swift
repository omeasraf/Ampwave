...
public class CarPlaySceneDelegate: UIResponder, CPTemplateApplicationSceneDelegate {
    ...
    public func templateApplicationScene(_ scene: CPTemplateApplicationScene, didConnect controller: CPInterfaceController) {
    ...
    public func templateApplicationScene(_ scene: CPTemplateApplicationScene, didDisconnect controller: CPInterfaceController) {
    ...
    private func updateNowPlayingButtons() {
        ...
        let likeButton = CPNowPlayingImageButton(image: UIImage(systemName: isLiked ? "heart.fill" : "heart")!) { _ in
            Task { @MainActor in
    ...
    private func createLibraryTemplate() -> CPListTemplate {
        let items = [
            createLibraryNavigationItem(title: "Artists", systemImage: "music.mic") { [weak self] in self?.showArtists() },
            createLibraryNavigationItem(title: "Albums", systemImage: "square.stack") { [weak self] in self?.showAlbums() },
            createLibraryNavigationItem(title: "Songs", systemImage: "music.note") { [weak self] in self?.showAllSongs() }
        ]
    ...
    private func createLibraryNavigationItem(title: String, systemImage: String, action: @escaping () -> Void) -> CPListItem {
...
extension CarPlaySceneDelegate: CPNowPlayingTemplateObserver {
    ...
    public func nowPlayingTemplateAlbumArtistButtonTapped(_ nowPlayingTemplate: CPNowPlayingTemplate) {
        ...
        guard let song = PlaybackController.shared.currentItem,
              let album = library.albums.first(where: { $0.name == song.album && $0.artist == song.artist }) else {
            return
        }
...
extension CarPlaySceneDelegate: CPSearchTemplateDelegate {
    public func searchTemplate(_ searchTemplate: CPSearchTemplate, updatedSearchText searchText: String, completionHandler completion: @escaping ([CPListItem]) -> Void) {
    ...
    public func searchTemplate(_ searchTemplate: CPSearchTemplate, selectedResult item: CPListItem, completionHandler completion: @escaping () -> Void) {
