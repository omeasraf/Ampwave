...
//
  ...
  public class CarPlaySceneDelegate: UIResponder, CPTemplateApplicationSceneDelegate {
    ...
    private func createLibraryTemplate() -> CPListTemplate {
      let items = [
        createLibraryItem(
          title: "Songs", systemImage: "music.note", action: { [weak self] in self?.showAllSongs() }
        ),
        createLibraryItem(
          title: "Artists", systemImage: "music.mic", action: { [weak self] in self?.showArtists() }
        ),
        createLibraryItem(
          title: "Albums", systemImage: "square.stack",
          action: { [weak self] in self?.showAlbums() }),
      ]
    ...
    private func createLibraryItem(title: String, systemImage: String, action: @escaping () -> Void)
      -> CPListItem
    {
