...
struct SettingsView: View {
  ...
  var body: some View {
    ...
    .alert("Clear Cache?", isPresented: $showingClearCacheConfirmation) {
      ...
      Text("This will remove all cached artwork and lyrics. Your music files will not be affected.")
    }
  ...
  private var libraryStatsSection: some View {
    Section {
      ...
      HStack {
        ...
        Spacer()
        Text(formatListeningTime(historyTracker.getTotalListeningTime()))
          .foregroundStyle(.secondary)
      }
    } header: {
  ...
  private func resetLibrary() {
    ...
    Task {
      ...
      print("[DEBUG] SettingsView.resetLibrary: Reloading library and playlists")
