    ...
    var body: some View {
        NavigationStack {
            ZStack {
                ...
                if let lyrics = playback.currentLyrics, lyrics.hasLyrics {
                    ScrollViewReader { proxy in
                        ScrollView(.vertical, showsIndicators: false) {
                            VStack(spacing: 20) {
                                ...
                                ForEach(Array(lyrics.lines.enumerated()), id: \.element.timestamp) { index, line in
                                    Text(line.text)
                                        .font(.system(size: 22, weight: isCurrentLine(index) ? .bold : .semibold))
                                        .foregroundStyle(isCurrentLine(index) ? .white : .white.opacity(0.35))
                                        .multilineTextAlignment(.center)
                                        .fixedSize(horizontal: false, vertical: true)
                                        .lineSpacing(2)
                                        .padding(.horizontal, 24)
                                        .frame(maxWidth: .infinity, alignment: .center)
                                        .scaleEffect(isCurrentLine(index) ? 1.08 : 1.0, anchor: .center)
                                        .id(index)
                                        .onTapGesture {
                                            playback.seek(to: line.timestamp)
                                        }
                                        .animation(.spring(response: 0.3, dampingFraction: 0.7), value: playback.currentLyricIndex)
                                }
                        ...
                        .onChange(of: playback.currentLyricIndex) { _, newIndex in
                            guard let idx = newIndex, !isUserScrolling else { return }
                ...
                } else if let plainLyrics = playback.currentItem?.lyrics, !plainLyrics.isEmpty, !plainLyrics.isLRCFormatted {
                    // Plain text fallback
                    ScrollView(.vertical, showsIndicators: false) {
                        VStack(spacing: 20) {
...
private extension String {
    ...
    var isLRCFormatted: Bool {
        let lrcPattern = #/^\[\d{2}:\d{2}[.:]\d{2}\]/#
        return self.split(separator: "\n").prefix(5).contains { line in
            line.firstMatch(of: lrcPattern) != nil
        }
    }
}
                ...
                ScrollViewReader { proxy in
                    ScrollView(.vertical, showsIndicators: false) {
                        VStack(spacing: 12) {
                            ...
                            ForEach(Array(lyrics.lines.enumerated()), id: \.element.timestamp) { index, line in
                                Text(line.text)
                                    .font(.system(size: 15, weight: isCurrentLine(index) ? .bold : .regular))
