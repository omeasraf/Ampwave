  ...
  var body: some View {
    List {
      ...
      ForEach(filteredSongs) { song in
        ...
        .onTapGesture {
          playback.playQueue(
            filteredSongs, startingAt: filteredSongs.firstIndex(where: { $0.id == song.id }) ?? 0)
        }
    ...
    ScrollView {
      if filteredAlbums.isEmpty {
        ContentUnavailableView(
          ...
          description: Text(
            library.albums.isEmpty ? "Import songs to see albums" : "No albums match your search")
        )
        .padding(.top, 100)
      } else {
    ...
    List {
      ForEach(filteredArtists) { artist in
        NavigationLink(destination: ArtistView(artist: artist)) {
          HStack(spacing: 12) {
            ArtistImageView(artworkPath: artist.artworkPath, size: 50)
